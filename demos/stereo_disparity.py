...
# get a list of the left-images image files and sort them (by timestamp in filename)
...
for filename_left in left_file_list:
    ...
    # from the left-images image filename get the correspondoning right-images image
    ...
    # check the file is a PNG file (left-images) and check a correspondoning right-images image
    # actually exists
    ...
    if ('.png' in filename_left) and (os.path.isfile(full_path_filename_right)) :
        ...
        _, disparity = cv2.threshold(disparity,0, max_disparity * 16, cv2.THRESH_TOZERO);   # Actually modifies the shape of the image.
        disparity_scaled = (disparity / 16.).astype(np.uint8);

        # crop disparity to chop out left-images part where there are with no disparity
        ...
        if (key == ord('x')):       # exit
            ...
            cv2.imwrite("left-images.png", imgL);
            cv2.imwrite("right-images.png", imgR);
        elif (key == ord('c')):     # crop
